...
def generate_launch_description():
    ...
    pkg__this = get_package_share_directory('m6e3_imu')
    ...
    # Специально для IMU выводим терминал, чтобы смотреть, чего он там намерил!
    show_imu_outputs = ExecuteProcess(
        cmd=['gnome-terminal', '--geometry=36x86', '--', 'bash', '-c',
                "ros2 topic echo /imu"],
        output='screen',
    )

    # Ну и собственно всё это исполняем, м-даа...
